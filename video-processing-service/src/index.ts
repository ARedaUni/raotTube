...
// Initialise Firebase Admin with explicit credential loading
try {
  admin.initializeApp({
    credential: admin.credential.applicationDefault()
  });
...
app.post('/transcode', async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      await db.collection('videos').doc(sanitizedId).set({
      ...
      console.log(`Successfully wrote video metadata to Firestore for ${data.videoId}`);
    } catch (firestoreError) {
      console.error('Failed to write metadata to Firestore:', firestoreError);
